 ...
 */
template <typename T>
inline py::handle sp_mat_to_csc(arma::SpMat<T> const& src) {
    src.sync();
    ...
    PyObject* args = Py_BuildValue("((OOO))", data.ptr(), indices.ptr(), indPtr.ptr());
...
template <typename T>
struct type_caster<arma::SpMat<T>> {
   public:
    PYBIND11_TYPE_CASTER(arma::SpMat<T>, _("scipy.sparse.csc.csc_matrix[") + npy_format_descriptor<T>::name + _("]"));
    ...
    static handle cast(arma::SpMat<T> const& src, return_value_policy, handle) { return sp_mat_to_csc(src); }
};
}  // namespace pybind11::detail
